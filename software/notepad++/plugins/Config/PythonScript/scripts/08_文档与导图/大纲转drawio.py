# -*- coding: utf-8 -*-
"""
将当前缩进大纲（规则与「Mermaid脑图」相同）导出为 diagrams.net（draw.io）可用的 .drawio 文件。
弹窗选择保存路径。无需 pip；优先 tkinter，无则 Windows 下用 scripts/_lib/folder_dialog_win.py（PowerShell 另存为路径经 UTF-8 临时文件回传，避免中文路径乱码）。

首行非空为根，后续用行首空格/Tab 表示层级（Tab 按 4 列展开）。
"""
...
def outline_to_drawio():
    ...
    if root is None:
        notepad.messageBox("没有可导出的大纲。", "大纲转 draw.io", 0)
        return
    ...
    if ask_save_filename is None:
        notepad.messageBox(
            "未找到 scripts/_lib/folder_dialog_win.py（或上级目录中的 _lib）。",
            "大纲转 draw.io",
            0,
        )
        return
    ...
    try:
        ...
        notepad.messageBox("写入失败：\n{}".format(e), "大纲转 draw.io", 0)
        return

    notepad.messageBox("已保存：\n{}".format(path), "大纲转 draw.io", 0)
