...
folder_dialog_win.py under scripts_ENG/_lib (PowerShell returns path via UTF-8 temp file on Windows).
...
def outline_to_drawio():
    ...
    if ask_save_filename is None:
        notepad.messageBox(
            "Missing scripts_ENG/_lib/folder_dialog_win.py (or _lib in a parent folder).",
            "Outline to draw.io",
            0,
        )
        return
